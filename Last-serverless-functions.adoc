...
To create serverless functions you need to use `kn func create` command which generates a function project from a template. 
...
A local container image is created for the project using `kn func build` command.
...
. The `-r (or --registry)` flag adds the registry and namespace of the image registry.
...
c. To keep things simple, we are not writing integration tests for this applications. 
...
d. As we have removed the integration.js containing boilerplate integration test cases, hence we also need to edit the `npm test` script in `package.json` file. 

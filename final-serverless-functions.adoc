...
Before the initial build, the `image` property of the `func.yaml` file is empty. It defaults to the project name for the image name. Alternatively, an image name can be specified by using `-i` or `-r` flags. 
...
You have the option to use the `-b` (or `--build`) flag with the `run` command, letting it to automatically build the container image. This triggers the build only if the image wasn't built earlier.

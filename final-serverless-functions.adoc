...
When HTTP invocation format is selected, it handles the HTTP invocation procedure.  That includes running an HTTP server, exposing HTTP endpoints, parsing incoming data from request, invoking the function, and finally transforming the returned value into a proper HTTP response.
...
To get a breif overview of the development cycle of Serverless Functions, refer to the steps below :
...
. Then, execute the function locally with the `kn func run` subcommand and push the built image to a container image registry.
